...
def annotate_image(first_image: np.ndarray) -> np.ndarray:
    fig, ax = plt.subplots()
    ...
    def onclick(event):
        ...
        ax.scatter(x=[event.xdata], y=[event.ydata], color='red')
        fig.canvas.draw()
...
def run_model(model, rgbs, N, sw):
    first_image = rgbs[0][0].permute(1, 2, 0).numpy()
    xs, ys = annotate_image(first_image)
    ...
    x = torch.tensor([xs], device=torch.device('cpu'), dtype=torch.float32)
    y = torch.tensor([ys], device=torch.device('cpu'), dtype=torch.float32)
...
def main():
    ...
    filenames = glob.glob('./custom_images/*.jpg')
